...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            商品名称：<input name="pname" id="pname" value="${condition.pname}">&nbsp;&nbsp;&nbsp;
            商品类型：<select name="typeid" id="typeid">
            <option value="-1">请选择</option>
            <c:forEach items="${ptlist}" var="pt">
                <option value="${pt.typeId}"
                <c:if test="${condition.typeid==pt.typeId}">
                        selected="selected"
               </c:if>   >${pt.typeName}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
            价格：<input name="lprice" id="lprice" value="${condition.lprice}">-<input name="hprice" id="hprice" value="${condition.hprice}">
...
<script type="text/javascript">
    ...
    function one(pid, ispage) {
        ...
        var lprice=$("#lprice").val();
        var hprice=$("#hprice").val();
        var str="?pid="+pid+"&pname="+pname+"&typeid="+typeid+"&lprice="+lprice+"&hprice="+hprice+"&page="+ispage
